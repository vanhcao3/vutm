...
import './index.scss'
import { columnsType } from "../VTAutocomplete1";
            ...
            <tr className='customRenderRow'>
                {
                    columns.map((col, index) => {
                        if (!col.root) return <td key={col.key} className='p-2 text-center'>
                            {col.render ? col.render(personData) : personData[col.key]}
                        </td>
                        else return (
                            <td key='root' className='p-2 flex items-center min-h-[60px]'>
                                {
                                    typeCheckBoxNode &&
                                    <Checkbox
